...
def main() -> None:
    ...
    parser.add_argument(
        "--sp500", action="store_true", help="Run an S&P 500 simulation instead of custom tickers"
    )
    parser.add_argument(
        "--sp500-limit", type=int, default=25, help="Number of S&P 500 constituents to simulate"
    )
    parser.add_argument(
        "--fallback-sp500", action="store_true", help="Use bundled fallback S&P 500 sample"
    )
    parser.add_argument(
        "--include-news", action="store_true", help="Include NYT/Economist geopolitical evidence"
    )
    ...
    if args.sp500:
        ...
        if args.pdf_report:
            output = generate_pdf_report(
                simulation, args.pdf_report, simulation.news_evidence or []
            )
