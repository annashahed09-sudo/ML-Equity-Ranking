...
def information_coefficient(scores: pd.Series, future_returns: pd.Series) -> float:
    """
...
def compute_ic_by_date(
    df: pd.DataFrame, score_col: str = "model_score", return_col: str = "forward_return"
) -> pd.Series:
    """
...
def long_short_portfolio_returns(
    ...
    for date, group in df.groupby("date"):
        ...
        gross_return = (
            group[group["ticker"].isin(longs)][return_col].mean()
...
def summarize_portfolio_performance(returns_df: pd.DataFrame) -> dict:
    """
