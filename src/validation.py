...
def create_walk_forward_splits(
    ...
    dates_arr = df[date_column].to_numpy()

    def _snap_to_date_boundary(idx: int) -> int:
        """Move ``idx`` forward so the split never cuts through a single date.

        Rows are sorted by date, so a boundary sits where the date changes.
        Snapping guarantees an entire date lands wholly in train or test,
        which prevents same-date leakage across the train/test boundary.
        """
        if idx <= 0:
            return 0
        if idx >= n_rows:
            return n_rows
        while idx < n_rows and dates_arr[idx] == dates_arr[idx - 1]:
            idx += 1
        return idx
    ...
    for fold_id in range(n_splits):
        ...
        train_end_idx = _snap_to_date_boundary(test_end_idx - test_size)
        test_end_idx = _snap_to_date_boundary(test_end_idx)
        test_start_idx = train_end_idx

        if train_end_idx <= 0 or test_start_idx >= test_end_idx:
            continue  # degenerate fold after snapping; skip
...
def get_fold_data(df: pd.DataFrame, fold: WalkForwardFold) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def validate_fold_integrity(
    df: pd.DataFrame, folds: List[WalkForwardFold], date_column: str = "date"
) -> Tuple[bool, List[str]]:
    """
    ...
    for fold in folds:
        ...
        if train_df[date_column].max() >= test_df[date_column].min():
            issues.append(f"Fold {fold.fold_id}: train dates overlap test start date")
